...
function updateDisplay() {
    ...
    // Instead of total Price, have a budget that you remove money from. 
    // If you try adding something and there isn't enough money in the budget, have it not add the item/ make the budget negative. 

    // Update Budget Display
    budgetDisplay.innerHTML = "Budget: $" + budget;
...
//document.getElementById("userBudget").addEventListener("change", changeBudget(userBudget));
var userBudgetInput = document.getElementById("userBudget");

// Function to change the budget to what to user inputs.
function getUserBudget() {
  var userBudget = document.getElementById("userBudgetDisplay").value;
  budget = userBudget;
  budgetDisplay.innerHTML = "Budget: $" + budget;
}
    ...
    for (var i = 0; i < equipment.length; i++) {
      if (equipment[i].name == item) {
        if (equipment[i].number < equipment[i].max) {
            if (equipment[i].price <= budget) {
                equipment[i].number++;
                budget -= equipment[i].price;
                updateDisplay();
                //alert("Add Function works!");
            }
            else {
                //alert("doesn't fit in your budget!");
            }
        }
...
// Change the budget by button amount and reset values to starting values.
function changeBudget(amount) {
    // Change budget to button amount
    budget = amount;
    // Reset items back to base amounts
    for (var i = 0; i < equipment.length; i++) {
        equipment[i].number = equipment[i].min;
    }
    updateDisplay();
}
